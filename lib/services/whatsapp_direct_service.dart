...
class WhatsAppDirectService {
  ...
  // PRIVATE METHODS UNTUK GENERATE CAPTION - UPDATED dengan Body Age Assessment
...
Body Age: ${data.bodyAge} tahun vs Usia Asli: ${data.age} tahun
...
• Body Age: ${data.bodyAge} tahun (${data.bodyAgeAssessment})
...
• Body Age Assessment: ${data.bodyAgeAssessment}

*SEGMENTAL ANALYSIS:*
• Trunk: ${(data.segmentalSubcutaneousFat['trunk'] ?? 0.0).toStringAsFixed(1)}%
• Arms: ${(data.segmentalSubcutaneousFat['arms'] ?? 0.0).toStringAsFixed(1)}%
• Legs: ${(data.segmentalSubcutaneousFat['legs'] ?? 0.0).toStringAsFixed(1)}%
...
• Body Age: ${data.bodyAge} years (${data.bodyAgeAssessment})

*SEGMENTAL DATA:*
• Trunk Fat: ${(data.segmentalSubcutaneousFat['trunk'] ?? 0.0).toStringAsFixed(1)}%
• Arms: ${(data.segmentalSubcutaneousFat['arms'] ?? 0.0).toStringAsFixed(1)}%
• Legs: ${(data.segmentalSubcutaneousFat['legs'] ?? 0.0).toStringAsFixed(1)}%

*MUSCLE DISTRIBUTION:*
• Trunk Muscle: ${(data.segmentalSkeletalMuscle['trunk'] ?? 0.0).toStringAsFixed(1)}%
• Arms Muscle: ${(data.segmentalSkeletalMuscle['arms'] ?? 0.0).toStringAsFixed(1)}%
• Legs Muscle: ${(data.segmentalSkeletalMuscle['legs'] ?? 0.0).toStringAsFixed(1)}%

*Overall Assessment:* ${data.overallAssessment}

${_generateTrainerRecommendation(data)}
...
Generated by LSHC Omron App
  ...
  // TAMBAHKAN: Trainer specific recommendation berdasarkan body age
  static String _generateTrainerRecommendation(OmronData data) {
    if (data.bodyAgeAssessment == 'Sangat Tua' || data.bodyAgeAssessment == 'Lebih Tua') {
      return '''
🎯 *TRAINING FOCUS:*
• Anti-aging program needed
• Cardio + Strength training
• Flexibility & mobility work
• Nutrition consultation required''';
    } else if (data.bodyAgeAssessment == 'Sangat Muda' || data.bodyAgeAssessment == 'Lebih Muda') {
      return '''
🎯 *TRAINING FOCUS:*
